...
\class dmz::ParserJSON
\ingroup Foundation
\brief JSON Parser.
\details This class parses an JSON stream and passes the result to a dmz::InterpreterJSON.
\sa dmz::InterpreterJSON

*/

struct dmz::ParserJSON::State {
...
dmz::ParserJSON::ParserJSON () : _state (*(new State)) {;}


dmz::ParserJSON::~ParserJSON () {
...
\brief Sets current InterpreterJSON.
\param[in] interpreter Pointer to the InterpreterJSON to use to interpret the parsed JSON.

*/
void
dmz::ParserJSON::set_interpreter (InterpreterJSON *interpreter) {
...
//! Resets the parser.
void
dmz::ParserJSON::reset () { _state.firstParse = True; _state.error.flush (); }
...
dmz::String
dmz::ParserJSON::get_error () { return _state.error; }

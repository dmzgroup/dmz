...
namespace {
   ...
   struct ParseStruct {
      ...
      dmz::Boolean error;
      dmz::InterpreterXML *interpreter;
      XML_Parser parser;
...
\ingroup Foundation
\brief XML Parser.
\details This class parses an XML stream and passes the result to a dmz::InterpreterXML.
\sa dmz::InterpreterXML

*/

struct dmz::ParserXML::State {
...
dmz::ParserXML::ParserXML () : _state (*(new State)) {;}


dmz::ParserXML::~ParserXML () {
...
\brief Sets current InterpreterXML.
\param[in] interpreter Pointer to the InterpreterXML to use to interpret the parsed XML.

*/
void
dmz::ParserXML::set_interpreter (InterpreterXML *interpreter) {
...
//! Resets the parser.
void
dmz::ParserXML::reset () { _state.firstParse = True; _state.error.flush (); }
...
dmz::String
dmz::ParserXML::get_error () { return _state.error; }

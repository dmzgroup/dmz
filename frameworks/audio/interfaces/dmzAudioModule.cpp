...
\fn dmz::Handle dmz::AudioModule::create_sound (const dmz::String &FileName)
...
\fn dmz::Boolean dmz::AudioModule::destroy_sound (const dmz::Handle AudioHandle)
...
\param[in] Init Contains the initialization parameters of the sound.
...
\note The instance is released as soon as the sound stops playing. If the sound is looped,
...
const dmz::Handle SoundHandle,
const dmz::SoundAttributes &Attributes)
\brief Updates a sound instance's attributes.
\param[in] SoundHandle Handle of the sound instance being updated.
...
\param[in] SoundHandle Handle of the sound instance to retrieve the attributes.
\param[out] init Used to return the initialization parameters of the sound instance.
\param[out] attributes Used to return the attributes of the sound instance.
\return Returns dmz::True if the attributes for the specified sound instance can be found.

\fn dmz::Boolean dmz::AudioModule::stop_sound (const dmz::Handle SoundHandle)
\brief Stops an audio instance.
\details Function can be used with both looped and single play sounds.
\param[in] SoundHandle Handle of the sound instance to stop.

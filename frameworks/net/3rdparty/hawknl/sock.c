...
/* Note: Windows does not seem to support the sharing of ports -REB */
    if(setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, (char *)&i, (int)sizeof(i)) == SOCKET_ERROR)
    {
        nlSetError(NL_SYSTEM_ERROR);
    }
#else
/* Note: we want to reuse PORT as well as Address -REB */
    if(setsockopt(socket, SOL_SOCKET, SO_REUSEPORT, (char *)&i, (int)sizeof(i)) == SOCKET_ERROR)
    {
        nlSetError(NL_SYSTEM_ERROR);
    }
#endif

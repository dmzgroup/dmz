...
\class dmz::HashTable$(type)Iterator
\ingroup Types
\brief Use to iterate over hash table elements.
\sa dmz::HashTable$(type)Template::get_first \n
dmz::HashTable$(type)Template::get_next \n
dmz::HashTable$(type)Template::get_prev \n
dmz::HashTable$(type)Template::get_last \n

*/

struct dmz::HashTable$(type)Iterator::idata {
...
//! Constructor.
dmz::HashTable$(type)Iterator::HashTable$(type)Iterator () : data (*(new idata)) {;}


//! Destructor.
dmz::HashTable$(type)Iterator::~HashTable$(type)Iterator () { delete &data; }


//! Gets hash key of current item returned from iteration.
dmz::$(type)
dmz::HashTable$(type)Iterator::get_hash_key () { return data.key; }


//! Resets iterator.
void
dmz::HashTable$(type)Iterator::reset () { data.index = -1; }


/*!

\class dmz::HashTable$(type)
\ingroup Types
\brief Internal class used by dmz::HashTable$(type)Template.
\note This Class should not be used directly. Use dmz::HashTable$(type)Template instead.

*/

struct dmz::HashTable$(type)::State {
   ...
   Boolean find_index (const $(type) &Key, Int32 &index) const {
...
//! Constructor.
dmz::HashTable$(type)::HashTable$(type) () : _state (*(new State ())) {;}


//! Constructor.
dmz::HashTable$(type)::HashTable$(type) (
      const Int32 Size,
      const UInt32 Mask) :
      _state (*(new State ())) {
...
dmz::Int32
dmz::HashTable$(type)::get_size () const { return _state.size; }
...
void *
dmz::HashTable$(type)::get_next (HashTable$(type)Iterator &it, const Boolean Prev) const {
...
void *
dmz::HashTable$(type)::lookup (const $(type) &Key) const {
...
dmz::Boolean
dmz::HashTable$(type)::store (const $(type) &Key, void *data) {
...
void *
dmz::HashTable$(type)::remove (const $(type) &Key) {
...
void
dmz::HashTable$(type)::grow (const Int32 Size) {
...
void
dmz::HashTable$(type)::set_table_size (const Int32 Size) {
...
void
dmz::HashTable$(type)::set_attributes (const UInt32 Attributes) {
...
dmz::UInt32
dmz::HashTable$(type)::get_attributes () const {
